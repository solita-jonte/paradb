...
class TestNoRebalanceWithOneShard:
    @pytest.mark.asyncio
    async def test_no_partition_moved(self):
...
class TestNoRebalanceWhenDifferenceLessThan2:
    @pytest.mark.asyncio
    async def test_nothing_changes(self):
...
class TestRebalanceMovesPartition:
    @pytest.mark.asyncio
    async def test_moves_from_biggest_to_smallest(self):
        ...
        # when we balance
        with patch("orchestrator.balancer.ShardCommand") as MockCmd:
            MockCmd.return_value.halt_flush_partition_writes = AsyncMock()
            MockCmd.return_value.send_partitions = AsyncMock()
            with patch("orchestrator.balancer.ShardBroadcastCommand") as MockBroadcast:
                MockBroadcast.return_value.send_partitions = AsyncMock()
                await balance_shards()
...
class TestRebalanceMovesOnlyOnePartition:
    @pytest.mark.asyncio
    async def test_exactly_one_moved(self):
...
class TestRebalanceHaltsWritesBeforeMoving:
    @pytest.mark.asyncio
    async def test_halt_called_before_reassignment(self):
        ...
        with patch("orchestrator.balancer.ShardCommand") as MockCmd:
            async def track_halt(partition_index):
            ...
            with patch("orchestrator.balancer.ShardBroadcastCommand") as MockBroadcast:
                ...
                MockBroadcast.return_value.send_partitions = track_broadcast
                await balance_shards()
